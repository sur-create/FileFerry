...
rm -rf "$BUILD_DIR"
mkdir -p "$PKG_ROOT/DEBIAN" "$PKG_ROOT/opt/fileferry" "$PKG_ROOT/usr/bin" "$PKG_ROOT/usr/share/applications" "$INSTALLER_DIR"
...
Depends: libc6
Description: FileFerry LAN single-file transfer tool
 Self-contained single-file transfer utility for LAN environments.
CONTROL
